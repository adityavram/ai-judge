/**
 * Per-client daily rate limiting for pipeline runs.
 *
 * Uses X-Client-Id header (a UUID generated by the client and stored in localStorage)
 * to track usage. Each POST /api/pipeline increments the counter; GET polls don't count.
 *
 * Default: 5 rounds per 24-hour window, configurable via DAILY_ROUND_LIMIT env var.
